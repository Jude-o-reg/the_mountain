...
void add_noise(float **matrix, int height, int width, int number, int max_size){
    ...
    for (i=0;i<number;i++){
        int center_x = max_size + rand()%(width-max_size); // initial position
        int center_y = max_size + rand()%(height-max_size);
        int x, y;
        int startx, starty, endx, endy;
        int radius = 1 + rand()%(max_size-1); // at least 1 pixel radius
        int lower = rand()%2; // the plateau can bring the terrain down or up
...
void generate_landscape(int seed){
    ...
    landscape_width = MIN_WIDTH + rand()%MIN_WIDTH;
    landscape_height = MIN_WIDTH + rand()%MIN_WIDTH;
    ...
    add_peak(matrix, landscape_height, landscape_width,
            rand()%landscape_width, // peak x position
            rand()%landscape_height, // peak y position
            400); // peak height
...
int main(){
    ...
    // Run this for performance testing, result is avg over 100 runs
    // Comment out if you want single run
    printf("Performance results: %0.2f\n", performance_eval());
    return 0;

    // Run this for a single test
    printf("Single run\n");
    res = single_run(279);   // Set your seed here for testing
    printf("Done\n");
    //print_matrix();
    // The following shows the path taken, good for debugging
    printf("Path taken: ");
